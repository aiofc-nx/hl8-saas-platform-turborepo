...
Rationale: 新增错误处理与日志记录原则，规范企业级应用的错误处理最佳实践，确保业务逻辑清晰、错误传播正确、系统可观测性良好

Modified Sections:

- NEW: X. 错误处理与日志记录原则 - 新增企业级错误处理最佳实践

Added Sections:

- 职责分离原则（异常用于业务逻辑，日志用于监控调试）
- 错误处理层次（数据层、业务层、控制器层的错误处理规范）
- 日志记录规范（结构化日志、日志级别、敏感信息保护）
- 异常设计规范（业务异常类、异常层次结构）
- 反模式禁止（禁止用日志替代异常、禁止异常中不记录日志、禁止日志中不抛出异常）
- 测试要求（异常测试、日志测试）
- 监控和告警（错误监控、日志分析）
...
⚠️ plan-template.md - 需要在 Constitution Check 中添加错误处理与日志记录原则检查项
⚠️ spec-template.md - 需要在技术约束中添加错误处理规范要求
⚠️ tasks-template.md - 需要在代码审查任务中包含错误处理模式检查
...
- ✅ 在架构原则中增加充血模型要求
- ✅ 修复日志序列化问题：统一错误处理模式，实现"异常优先，日志辅助"原则
...
- ⚠️ 为团队提供 DDD 充血模型培训
- ⚠️ 更新代码审查检查清单，添加错误处理模式检查项
- ⚠️ 为团队提供错误处理最佳实践培训
- ⚠️ 审查现有代码库，确保错误处理符合"异常优先，日志辅助"原则
- 文档来源：docs/ts-config.md、docs/any-except.md、docs/backend-project-template.md、docs/XS-logging-serialization-fastify-pino.md
