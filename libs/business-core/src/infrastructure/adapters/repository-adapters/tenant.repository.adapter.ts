...
export class TenantRepositoryAdapter implements ITenantRepository {
   ...
   */
  async findById(id: EntityId, includeDeleted: boolean = false): Promise<TenantAggregate | null> {
    this.logger.info("查找租户", {
    ...
    try {
      ...
      try {
        const tenantData = await this.findTenantDataById(queryRunner, id, includeDeleted);
  ...
  async findByPlatform(
    ...
    try {
      ...
      try {
        const tenantsData = await this.findTenantsDataByPlatform(queryRunner, platformId, options);
  ...
  async findByName(
    ...
    try {
      ...
      try {
        const tenantData = await this.findTenantDataByName(queryRunner, platformId, name, includeDeleted);
  ...
  async existsByName(
    ...
    try {
      ...
      try {
        const exists = await this.checkTenantNameExists(queryRunner, platformId, name, excludeId);
        
        this.logger.info("租户名称存在性检查完成", {
          platformId: platformId.toString(),
          name,
          exists,
        });
      ...
      this.logger.error("检查租户名称是否存在失败", {
        platformId: platformId.toString(),
        name,
        error: error.message,
      });
  ...
  async countByPlatform(
    ...
    try {
      ...
      try {
        const count = await this.countTenantsByPlatform(queryRunner, platformId, options);
        
        this.logger.info("平台租户数量统计完成", {
          platformId: platformId.toString(),
          count,
        });
  ...
  async restore(id: EntityId, restoredBy: string): Promise<void> {
    ...
    try {
      ...
      // 检查名称唯一性
      const nameExists = await this.existsByName(tenant.platformId, tenant.tenant.name);
      if (nameExists) {
      ...
      this.logger.info("租户恢复成功", {
        tenantId: id.toString(),
        restoredBy,
      });
    } catch (error) {
      this.logger.error("恢复租户失败", {
        tenantId: id.toString(),
        error: error.message,
      });
      throw error;
   ...
   * @returns Promise<boolean>
   */
  async exists(id: EntityId, includeDeleted: boolean = false): Promise<boolean> {
    this.logger.info("检查租户是否存在", {
      tenantId: id.toString(),
      includeDeleted,
    });
    ...
    try {
      ...
      try {
        const exists = await this.checkTenantExists(queryRunner, id, includeDeleted);
        
        this.logger.info("租户存在性检查完成", {
          tenantId: id.toString(),
          exists,
        });
  ...
  // 私有方法：保存租户数据
  private async saveTenantData(queryRunner: any, tenantData: any): Promise<any> {
    // TODO: 实现具体的数据库保存逻辑
    throw new Error("方法未实现");
  }

  // 私有方法：根据ID查找租户数据
  private async findTenantDataById(queryRunner: any, id: EntityId, includeDeleted: boolean): Promise<any> {
    // TODO: 实现具体的数据库查询逻辑
    throw new Error("方法未实现");
  }

  // 私有方法：根据平台ID查找租户数据列表
  private async findTenantsDataByPlatform(queryRunner: any, platformId: EntityId, options: any): Promise<any[]> {
    // TODO: 实现具体的数据库查询逻辑
    throw new Error("方法未实现");
  }

  // 私有方法：根据名称查找租户数据
  private async findTenantDataByName(queryRunner: any, platformId: EntityId, name: string, includeDeleted: boolean): Promise<any> {
    // TODO: 实现具体的数据库查询逻辑
    throw new Error("方法未实现");
  }

  // 私有方法：检查租户名称是否存在
  private async checkTenantNameExists(queryRunner: any, platformId: EntityId, name: string, excludeId?: EntityId): Promise<boolean> {
    // TODO: 实现具体的数据库查询逻辑
    throw new Error("方法未实现");
  }

  // 私有方法：统计租户数量
  private async countTenantsByPlatform(queryRunner: any, platformId: EntityId, options: any): Promise<number> {
    // TODO: 实现具体的数据库查询逻辑
    throw new Error("方法未实现");
  }

  // 私有方法：检查租户是否存在
  private async checkTenantExists(queryRunner: any, id: EntityId, includeDeleted: boolean): Promise<boolean> {
    // TODO: 实现具体的数据库查询逻辑
    throw new Error("方法未实现");
  }
