...
describe("ActivateUserUseCase", () => {
  ...
  describe("构造函数", () => {
    it("应该正确初始化用例", () => {
      ...
      expect(useCase.useCaseName).toBe("ActivateUser");
      expect(useCase.useCaseDescription).toBe("激活用户用例");
      expect(useCase.useCaseVersion).toBe("1.0.0");
      expect(useCase.requiredPermissions).toEqual(["user:activate"]);
    });
  });
  ...
  describe("execute - 成功场景", () => {
    ...
    it("应该保存用户状态到仓储", async () => {
      ...
      expect(savedUser.getUser().status).toBe(UserStatus.ACTIVE);
    });
  ...
  describe("execute - 验证失败场景", () => {
    ...
    it("应该拒绝空激活者", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe("ACTIVATED_BY_REQUIRED");
    });
  });

  describe("execute - 业务规则验证", () => {
    it("应该拒绝激活已激活的用户", async () => {
      // 准备测试数据 - 先创建一个已激活的用户
      ...
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe("USER_ALREADY_ACTIVE");
    });

    it("应该拒绝激活已删除的用户", async () => {
      // 准备测试数据 - 先创建一个已删除的用户
