...
export class RateLimitService {
  ...
  private readonly memoryStore = new Map<string, { count: number; expireAt: number }>();

  constructor(private readonly options: RateLimitOptions) {
    this.logger.log(`速率限制服务已初始化，策略: ${options.strategy || 'ip'}, 限制: ${options.max}/${options.timeWindow}ms`);
  }
  ...
  async check(req: FastifyRequest<any>): Promise<RateLimitStatus> {
    try {
      ...
      this.logger.error(`速率限制检查失败: ${(error as Error).message}`, (error as Error).stack);
  ...
  generateKey(req: FastifyRequest<any>): string {
    ...
    try {
      ...
      this.logger.error(`键生成失败: ${(error as Error).message}`, (error as Error).stack);
      // 降级为 IP 策略
      return `ratelimit:ip:${req.ip}`;
    }
  }
  ...
  async getStatus(req: FastifyRequest<any>): Promise<RateLimitStatus> {
    ...
    if (this.options.redis) {
      ...
      return {
        ...
        remaining: Math.max(0, this.options.max - (count ? parseInt(count, 10) : 0)),
        total: this.options.max,
        ttl: ttl > 0 ? ttl : this.options.timeWindow,
        key,
      };
