...
describe('CacheKey', () => {
  ...
  describe('forTenant()', () => {
    it('应该创建租户级缓存键', () => {
      ...
      expect(key.toString()).toBe(`${PREFIX}tenant:${UUID_TENANT}:config:flags`);
  ...
  describe('forOrganization()', () => {
    it('应该创建组织级缓存键', () => {
      ...
      expect(key.toString()).toBe(`${PREFIX}tenant:${UUID_TENANT}:org:${UUID_ORG}:members:list`);
  ...
  describe('forUser()', () => {
    it('应该创建用户级缓存键', () => {
      ...
      expect(key.toString()).toBe(`${PREFIX}user:${UUID_USER}:preferences:theme`);
