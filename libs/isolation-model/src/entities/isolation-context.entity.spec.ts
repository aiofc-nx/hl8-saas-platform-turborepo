...
describe('IsolationContext', () => {
  ...
  describe('级别判断方法', () => {
    ...
    it('isOrganizationLevel() 应该正确判断', () => {
      expect(IsolationContext.organization(t123, o456).isOrganizationLevel()).toBe(true);
      expect(IsolationContext.tenant(t123).isOrganizationLevel()).toBe(false);
    });
    
    it('isDepartmentLevel() 应该正确判断', () => {
      expect(IsolationContext.department(t123, o456, d789).isDepartmentLevel()).toBe(true);
      expect(IsolationContext.organization(t123, o456).isDepartmentLevel()).toBe(false);
    });
  ...
  describe('buildCacheKey()', () => {
    ...
    it('应该为部门级生成正确的键', () => {
      ...
      expect(key).toBe(`tenant:${UUID_TENANT}:org:${UUID_ORG}:dept:${UUID_DEPT}:user:list`);
    });
    ...
    it('应该为用户级生成正确的键（有租户）', () => {
      ...
      expect(key).toBe(`tenant:${UUID_TENANT}:user:${UUID_USER}:preferences:theme`);
    });
  });
  ...
  describe('canAccess() - 核心业务逻辑', () => {
    describe('平台级上下文访问', () => {
      it('应该可以访问所有数据', () => {
        ...
        expect(platformContext.canAccess(dataContext, true, SharingLevel.TENANT)).toBe(true);
      });
    });
    ...
    describe('非共享数据访问', () => {
      ...
      it('应该拒绝访问不匹配的租户数据', () => {
        const userContext = IsolationContext.tenant(t123);
        const dataContext = IsolationContext.tenant(TenantId.create(UUID_TENANT_2));
      ...
      it('应该拒绝访问不匹配的组织数据', () => {
        ...
        const dataContext = IsolationContext.organization(t123, OrganizationId.create(UUID_ORG_2));
      ...
      it('应该拒绝访问不匹配的部门数据', () => {
        ...
        const dataContext = IsolationContext.department(t123, o456, DepartmentId.create(UUID_DEPT_2));
    ...
    describe('共享数据访问 - PLATFORM 级别', () => {
      it('应该允许所有用户访问平台共享数据', () => {
        ...
        expect(userContext.canAccess(dataContext, true, SharingLevel.PLATFORM)).toBe(true);
      });
    });
    ...
    describe('共享数据访问 - TENANT 级别', () => {
      it('应该允许同租户用户访问租户共享数据', () => {
        ...
        expect(userContext.canAccess(dataContext, true, SharingLevel.TENANT)).toBe(true);
      });
      
      it('应该拒绝不同租户用户访问', () => {
        const userContext = IsolationContext.tenant(TenantId.create(UUID_TENANT_2));
        const dataContext = IsolationContext.tenant(t123);
        
        expect(userContext.canAccess(dataContext, true, SharingLevel.TENANT)).toBe(false);
      });
    });
    ...
    describe('共享数据访问 - ORGANIZATION 级别', () => {
      it('应该允许同组织用户访问组织共享数据', () => {
        ...
        expect(userContext.canAccess(dataContext, true, SharingLevel.ORGANIZATION)).toBe(true);
      });
      
      it('应该拒绝不同组织用户访问', () => {
        const userContext = IsolationContext.organization(t123, OrganizationId.create(UUID_ORG_2));
        const dataContext = IsolationContext.organization(t123, o456);
        
        expect(userContext.canAccess(dataContext, true, SharingLevel.ORGANIZATION)).toBe(false);
      });
    });
    ...
    describe('共享数据访问 - DEPARTMENT 级别', () => {
      it('应该允许同部门用户访问部门共享数据', () => {
        ...
        expect(userContext.canAccess(dataContext, true, SharingLevel.DEPARTMENT)).toBe(true);
      });
      
      it('应该拒绝不同部门用户访问', () => {
        const userContext = IsolationContext.department(t123, o456, DepartmentId.create(UUID_DEPT_2));
        const dataContext = IsolationContext.department(t123, o456, d789);
        
        expect(userContext.canAccess(dataContext, true, SharingLevel.DEPARTMENT)).toBe(false);
      });
    });
    ...
    describe('共享数据访问 - USER 级别', () => {
      it('应该允许同一用户访问', () => {
        ...
        expect(userContext.canAccess(dataContext, true, SharingLevel.USER)).toBe(true);
      });
      ...
      it('应该拒绝不同用户访问', () => {
        ...
        expect(userContext.canAccess(dataContext, true, SharingLevel.USER)).toBe(false);
      });
    });
  ...
  describe('switchOrganization()', () => {
    it('应该创建新的上下文（不可变性）', () => {
      ...
      const newContext = original.switchOrganization(OrganizationId.create(UUID_ORG_2));
      
      expect(newContext).not.toBe(original);
      const newOrg = OrganizationId.create('123e4567-e89b-42d3-a456-426614174001');
    ...
    it('应该要求租户上下文', () => {
      ...
      expect(() => userContext.switchOrganization(o456)).toThrow(IsolationValidationError);
      expect(() => userContext.switchOrganization(o456)).toThrow('切换组织需要租户上下文');
    });
  });
  ...
  describe('switchDepartment()', () => {
    it('应该创建新的上下文', () => {
      ...
      const newContext = original.switchDepartment(DepartmentId.create(UUID_DEPT_2));
      
      expect(newContext).not.toBe(original);
      const newDept = DepartmentId.create('123e4567-e89b-42d3-a456-426614174002');
    ...
    it('应该要求租户和组织上下文', () => {
      ...
      expect(() => tenantContext.switchDepartment(d789)).toThrow(IsolationValidationError);
    });
  });
