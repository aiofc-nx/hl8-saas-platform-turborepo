  ...
  describe('UUID v4 验证', () => {
    ...
    it('应该拒绝非 UUID v4 格式', () => {
      ...
      invalidFormats.forEach(format => {
        expect(() => TestEntityId.create(format)).toThrow(IsolationValidationError);
      });
    });
    
    it('应该拒绝 null 和 undefined', () => {
      expect(() => TestEntityId.create(null as any)).toThrow(IsolationValidationError);
      expect(() => TestEntityId.create(undefined as any)).toThrow(IsolationValidationError);
    });
    ...
    it('应该提供正确的错误信息', () => {
      try {
        ...
        expect((error as IsolationValidationError).code).toBe('INVALID_TESTENTITY_FORMAT');
        expect((error as IsolationValidationError).message).toContain('UUID v4');
      }
    });
  });
