...
describe('TenantId', () => {
  ...
  describe('UUID v4 验证规则', () => {
    ...
    it('应该拒绝 null 和 undefined', () => {
      expect(() => TenantId.create(null as any)).toThrow(IsolationValidationError);
      expect(() => TenantId.create(undefined as any)).toThrow(IsolationValidationError);
    });
    
    it('应该拒绝非字符串类型', () => {
      expect(() => TenantId.create(123 as any)).toThrow(IsolationValidationError);
      expect(() => TenantId.create({} as any)).toThrow(IsolationValidationError);
    });
    ...
    it('应该拒绝非 UUID 格式的字符串', () => {
      ...
      invalidFormats.forEach(format => {
        ...
        expect(() => TenantId.create(format)).toThrow('TenantId 必须是有效的 UUID v4 格式');
      });
    });
    ...
    it('应该验证错误代码正确', () => {
      try {
        ...
        expect((error as IsolationValidationError).code).toBe('INVALID_TENANTID');
      }
      
      try {
        TenantId.create('invalid-uuid');
      } catch (error) {
        expect((error as IsolationValidationError).code).toBe('INVALID_TENANTID_FORMAT');
      }
    });
  });
  ...
  describe('Flyweight 模式性能', () => {
    it('应该缓存常用的 ID', () => {
      // 创建 100 次相同的 ID
      const ids = Array.from({ length: 100 }, () => TenantId.create(validUuid1));
