...
class TestUseCase extends BaseUseCase<TestRequest, TestResponse> {
  constructor(permissions: string[] = [], logger?: FastifyLoggerService) {
...
describe("BaseUseCase", () => {
  ...
  describe("异常处理方法", () => {
    ...
    it("应该能够抛出权限异常", () => {
      // Act & Assert
      expect(() =>
        (useCase as any).throwPermissionError("权限不足", ["admin"], ["user"], {
          detail: "test",
        }),
      ).toThrow();
    });
    ...
    it("应该能够抛出业务异常", () => {
      // Act & Assert
      expect(() =>
        (useCase as any).throwBusinessError("业务规则违反", "rule-1", {
          detail: "test",
        }),
      ).toThrow();
    });
    ...
    it("应该能够抛出执行异常", () => {
      // Act & Assert
      expect(() =>
        (useCase as any).throwExecutionError("执行失败", "operation-1", {
          detail: "test",
        }),
      ).toThrow();
    });
