...
async function fixEntityIdUsage(filePath) {
  ...
  // 3. 修复导入语句
  if (content.includes('EntityId.create') || content.includes('EntityId.generate')) {
    // 如果文件中使用了 EntityId 的方法，添加 TenantId 导入
    if (content.includes("import { EntityId") && !content.includes("TenantId")) {
      content = content.replace(
  ...
  if (hasChanges) {
    ...
    console.log(`✅ Fixed EntityId usage in: ${filePath.replace(REPO_ROOT, '.')}`);
  }
  return hasChanges;
}
