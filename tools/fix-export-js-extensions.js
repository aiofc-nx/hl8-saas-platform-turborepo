 ...
 */
async function fixExportJsExtensionsInFile(filePath) {
  let content = await readFile(filePath, 'utf-8');
  let hasChanges = false;

  // 修复 export ... from 语句中的相对路径，添加 .js 扩展名
  ...
  if (hasChanges) {
    ...
    console.log(`✅ Fixed export .js extensions in: ${relative(REPO_ROOT, filePath)}`);
  }
  return hasChanges;
}

async function main() {
  console.log('🚀 开始修复导出语句中缺失的 .js 扩展名...');
  ...
  for (const file of tsFiles) {
    if (await fixExportJsExtensionsInFile(file)) {
      fixedCount++;
    }
  }
