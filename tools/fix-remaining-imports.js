 ...
 */
async function fixRemainingImportsInFile(filePath) {
  let content = await readFile(filePath, 'utf-8');
  let hasChanges = false;

  // 修复所有相对导入路径，添加 .js 扩展名
  // 这个正则表达式更精确，只匹配没有扩展名的相对路径
  content = content.replace(
    /(import\s+[^'"]*from\s+['"])(\.{1,2}\/[^'"\s]*?)(['"])/g,
  ...
  if (hasChanges) {
    ...
    console.log(`✅ Fixed remaining imports in: ${relative(REPO_ROOT, filePath)}`);
  }
  return hasChanges;
}

async function main() {
  console.log('🚀 开始修复剩余的导入问题...');
  ...
  for (const file of tsFiles) {
    if (await fixRemainingImportsInFile(file)) {
      fixedCount++;
    }
  }
