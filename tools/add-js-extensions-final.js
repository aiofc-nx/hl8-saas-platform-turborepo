...
async function addJsExtensionsFinal(filePath) {
  let content = await readFile(filePath, 'utf-8');
  let hasChanges = false;

  // 匹配相对导入路径（以 ./ 或 ../ 开头），但不包括 node_modules 或绝对路径
  // 排除已经包含扩展名的导入
  const relativeImportRegex = /from\s+['"](\.[^'"]*?)(?<!\.js)(?<!\.ts)(?<!\.json)(?<!\.d\.ts)['"];?/g;
  ...
  content = content.replace(relativeImportRegex, (match, path) => {
    ...
    // 跳过已经包含扩展名的路径
    if (path.endsWith('.js') || path.endsWith('.ts') || path.endsWith('.json') || path.endsWith('.d.ts')) {
      return match;
    }
  ...
  // 修复 export 语句
  const relativeExportRegex = /from\s+['"](\.[^'"]*?)(?<!\.js)(?<!\.ts)(?<!\.json)(?<!\.d\.ts)['"];?/g;
...
async function main() {
  ...
  for (const file of files) {
    if (await addJsExtensionsFinal(file)) {
      fixedCount++;
    }
  }
