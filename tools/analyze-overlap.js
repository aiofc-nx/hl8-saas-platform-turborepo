...
function parseFileContent(filePath) {
  try {
    ...
    // 匹配类定义
    const classMatches = content.match(/export\s+(?:class|abstract class|interface|type|enum)\s+(\w+)/g);
    if (classMatches) {
      for (const match of classMatches) {
        const name = match.replace(/export\s+(?:class|abstract class|interface|type|enum)\s+/, '');
        items.push({
          type: 'class',
          name,
          file: relativePath,
    ...
    // 匹配值对象
    const valueObjectMatches = content.match(/export\s+class\s+(\w+)\s+extends\s+BaseValueObject/g);
    if (valueObjectMatches) {
      for (const match of valueObjectMatches) {
        const name = match.replace(/export\s+class\s+(\w+)\s+extends\s+BaseValueObject/, '$1');
        items.push({
          type: 'value-object',
          name,
          file: relativePath,
    ...
    // 匹配实体ID
    const entityIdMatches = content.match(/export\s+(?:class|type)\s+(EntityId|TenantId|UserId)/g);
    if (entityIdMatches) {
...
function analyzeOverlap() {
  ...
  for (const hybridItem of hybridItems) {
    const duplicate = isolationItems.find(item => item.name === hybridItem.name);
    if (duplicate) {
      overlaps.push({
