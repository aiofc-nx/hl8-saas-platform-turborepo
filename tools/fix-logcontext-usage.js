...
async function fixLogContextUsage(filePath) {
  ...
  if (content.includes('LogContext.')) {
    // 移除 LogContext 导入
    const logContextImportRegex = /import type\s*{\s*LogContext\s*}\s*from\s*['"]@hl8\/nestjs-fastify['"];?\s*\n?/g;
    ...
    if (logContextBusinessRegex.test(content)) {
      content = content.replace(logContextBusinessRegex, '{ context: "BUSINESS" }');
      hasChanges = true;
    }
    ...
    if (logContextDatabaseRegex.test(content)) {
      content = content.replace(logContextDatabaseRegex, '{ context: "DATABASE" }');
      hasChanges = true;
    }

    // 修复 logger 方法调用参数
    // 将 logger.method(message, LogContext.SYSTEM, context) 改为 logger.method(message, { ...context, context: "SYSTEM" })
    const loggerMethodRegex = /(\w+)\.(log|debug|info|warn|error)\(\s*([^,]+),\s*{\s*context:\s*["']([^"']+)["']\s*},\s*(\{[^}]+\})\s*\)/g;
    content = content.replace(loggerMethodRegex, (match, logger, method, message, contextType, context) => {
      hasChanges = true;
      return `${logger}.${method}(${message}, { ...${context}, context: "${contextType}" })`;
    });

    // 修复简单的 LogContext 使用
    const simpleLogContextRegex = /(\w+)\.(log|debug|info|warn|error)\(\s*([^,]+),\s*{\s*context:\s*["']([^"']+)["']\s*}\s*\)/g;
    content = content.replace(simpleLogContextRegex, (match, logger, method, message, contextType) => {
      hasChanges = true;
  ...
  if (hasChanges) {
    ...
    console.log(`✅ Fixed LogContext usage in: ${filePath.replace(REPO_ROOT, '.')}`);
  }
  return hasChanges;
}
