...
async function fixBrokenImports(filePath) {
  ...
  while (i < lines.length) {
    ...
    if (line.includes('import type {') && line.includes('import { TenantId }')) {
      // 分离 type import 和 TenantId import
      const typeImportMatch = line.match(/import type \{([^}]+)\}/);
      const tenantIdMatch = line.match(/import \{ TenantId \} from '@hl8\/isolation-model';/);
  ...
  if (hasChanges) {
    ...
    console.log(`✅ Fixed broken imports in: ${filePath.replace(REPO_ROOT, '.')}`);
  }
  return hasChanges;
}
