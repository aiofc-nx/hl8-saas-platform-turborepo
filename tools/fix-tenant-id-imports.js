...
async function fixTenantIdImports(filePath) {
  ...
  if (content.includes('TenantId') && !content.includes("import { TenantId }")) {
    // 在文件顶部添加 TenantId 导入
    ...
    lines.splice(insertIndex, 0, "import { TenantId } from '@hl8/isolation-model';");
    content = lines.join('\n');
    hasChanges = true;
  }
  ...
  if (hasChanges) {
    ...
    console.log(`✅ Fixed TenantId imports in: ${filePath.replace(REPO_ROOT, '.')}`);
  }
  return hasChanges;
}
