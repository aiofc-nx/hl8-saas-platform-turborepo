...
async function fixTypeImports(filePath) {
  ...
  const interfaceImportRegex = /import\s+\{\s*([^}]*I[A-Z][a-zA-Z]*[^}]*)\s*\}\s*from\s+['"][^'"]+\.js['"];?\s*$/gm;
  
  content = content.replace(interfaceImportRegex, (match, imports) => {
    // 检查是否只包含接口类型
    const interfaceTypes = ['I[A-Z][a-zA-Z]*', 'I[A-Z][a-zA-Z]*Service', 'I[A-Z][a-zA-Z]*Interface'];
  ...
  const typeExportRegex = /export\s+\{\s*([^}]*[A-Z][a-zA-Z]*[^}]*)\s*\}\s*from\s+['"][^'"]+['"];?\s*$/gm;
  ...
  if (hasChanges) {
    ...
    console.log(`✅ Fixed type imports in: ${filePath.replace(REPO_ROOT, '.')}`);
  }
  return hasChanges;
}
